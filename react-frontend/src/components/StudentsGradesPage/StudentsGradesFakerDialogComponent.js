...
import studentsGradesFakerFactory from "./studentsGradesFakerFactory";
...
const StudentsGradesFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = studentsGradesFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("studentsGrades").create(elem));
...
export default connect(mapState, mapDispatch)(StudentsGradesFakerDialogComponent);
