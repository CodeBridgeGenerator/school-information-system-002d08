...
import teachersFakerFactory from "./teachersFakerFactory";
...
const TeachersFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = teachersFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("teachers").create(elem));
...
export default connect(mapState, mapDispatch)(TeachersFakerDialogComponent);
