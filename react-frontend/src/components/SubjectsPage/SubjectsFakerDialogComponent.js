...
import subjectsFakerFactory from "./subjectsFakerFactory";
...
const SubjectsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = subjectsFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("subjects").create(elem));
...
export default connect(mapState, mapDispatch)(SubjectsFakerDialogComponent);
